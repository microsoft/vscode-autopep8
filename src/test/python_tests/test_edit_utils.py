...
@pytest.mark.parametrize(
    ...
    [
        (
            ...
            [
                lsp.TextEdit(
                    range=lsp.Range(lsp.Position(0, 10), lsp.Position(0, 11)),
                    new_text="\n",
                ),
            ],
        ),
...
def test_with_emojis2(encoding: lsp.PositionEncodingKind, expected: List[lsp.TextEdit]):
    FORMATTED_TEST_FILE_PATH = constants.TEST_DATA / "sample8" / "sample.py"
    UNFORMATTED_TEST_FILE_PATH = constants.TEST_DATA / "sample8" / "sample.unformatted"
